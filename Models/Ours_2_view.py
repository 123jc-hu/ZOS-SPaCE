...
class Model(nn.Module):
    def __init__(self, configs=None):
        ...
        # Adjust classifier input size based on fusion method
        # self.ClassifierBlock = self.classifier_block(self.Spatial_BlockOutputSize + self.Temporal_BlockOutputSize)
        # 使用全局平均池化和 1x1 卷积降维
        self.ClassifierBlock = nn.Sequential(
            nn.Conv1d(62+65, 32, kernel_size=1),  # 1x1 卷积降维
            nn.GELU(),
        ...
        #     nn.Flatten(),
        # )
        self.prototypes = nn.Parameter(torch.randn(self.n_class, 32) * 0.01)  # 小范围初始化

        # self.projection_head1 = nn.Sequential(
        #     Rearrange("b t f -> b 1 t f"),
        #     nn.Conv2d(1, 4, (65, 1), stride=(1, 1), bias=False),
        #     nn.GELU(),
        #     nn.Dropout(0.1),
        #     nn.Flatten(),
        # )
        # self.projection_head2 = nn.Sequential(
        #     Rearrange("b c f -> b 1 c f"),
        #     nn.Conv2d(1, 4, (62, 1), stride=(1, 1), bias=False),
        #     nn.GELU(),
        #     nn.Dropout(0.1),
        #     nn.Flatten(),
        # )
    ...
    def forward(self, x):
        ...
        x_s = self.Spatial_Block(x)  # (b, 32, 1, 65)
        # 调整维度
        x_t = x_t.squeeze()  # (b, 62, 32)
        x_s = x_s.squeeze()  # (b, 65, 32)
        ...
        x = torch.cat((x_t, x_s), dim=1)  # (b, 62 + 65, 32)
        # x = x.transpose(1, 2)  # (b, 32, 62 + 65)
        # x = self.hybrid_Transformer_Block(x)

        # x = self.activation(x)
        x = self.ClassifierBlock(x)

        # feature_t = self.projection_head2(x_t)
        # feature_s = self.projection_head1(x_s)

        # 归一化 x 和 prototypes
        x_norm = torch.norm(x, p=2, dim=1, keepdim=True)  # (batch, 1)
        x_normalized = x / (x_norm + 1e-5)  # (batch, dim)

        prototypes_norm = torch.norm(self.prototypes, p=2, dim=1, keepdim=True)  # (2, 1)
        prototypes_normalized = self.prototypes / (prototypes_norm + 1e-5)  # (2, dim)

        # 计算余弦相似度
        cosine_similarity = torch.matmul(x_normalized, prototypes_normalized.T)  # (batch, 2)
