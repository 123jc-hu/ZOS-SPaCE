...
class ConcreteMultiSelector(nn.Module):
    ...
    def forward(self, x: torch.Tensor, current_epoch: int = None):
        ...
        if self.training:
            ...
            # —— 原有(K,C)路径保持不变 ——
            P_soft = self._row_softmax(self.alpha, beta)  # (K,C)

            if current_epoch < self.warmup_epochs:
                # 阶段1：禁止选择全零通道
                P_soft_real = P_soft[:, :-1]  # (K, C)
                # 重新归一化，排除全零通道
                P_soft_real = P_soft_real / P_soft_real.sum(dim=1, keepdim=True)

                with torch.no_grad():
                    # 在真实通道中选择
                    idx_real = torch.argmax(self.alpha[:, :-1], dim=1)
                    W_hard = torch.zeros_like(P_soft_real)
                    W_hard[torch.arange(self.K), idx_real] = 1.0

                W = P_soft_real + (W_hard - P_soft_real).detach()
                z = torch.einsum("kc,bct->bkt", W, x[:, :, :-1].squeeze(1)).unsqueeze(1)

                return z, W, P_soft_real
            else:
                # 阶段2：允许选择全零通道
                with torch.no_grad():
    ...
    def orth_regularizer_new(self, P_soft: torch.Tensor) -> torch.Tensor:
        """
        加权正交约束：根据选择空通道的概率来降低约束强度
        """
        # 计算每行选择非空通道的总概率
        non_null_probs = P_soft[:, :-1].sum(dim=1)  # (K,)

        # 只对有效通道部分计算正交约束
        P_effective = P_soft[:, :-1]  # (K, C)
        G = P_effective @ P_effective.t()  # (K, K)
        I = torch.eye(self.K, device=G.device, dtype=G.dtype)

        # base_orth_loss = ((G - I) ** 2).sum()

        # 用选择非空通道的概率作为权重
        weight_matrix = non_null_probs.unsqueeze(1) * non_null_probs.unsqueeze(0)  # (K, K)
        weighted_orth_loss = (weight_matrix * (G - I) ** 2).sum()

        return weighted_orth_loss
...
class ModelWithSelector(nn.Module):
    ...
    def forward(self, x: torch.Tensor, current_epoch: int = None, **kwargs):
        ...
        z = z * self.post_scale
        hard_indices = torch.tensor(self.selected_indices())
        mask = hard_indices < self.C - 1  # 排除全零通道
        out = self.backbone(z, mask=mask, **kwargs)

        # if current_epoch and current_epoch < 20:
        #     # 创建mask版本：随机将部分真实通道置零
        #     z_masked = z.clone()
        #     batch_size, _, K, T = z.shape
        #
        #     for b in range(batch_size):
        #         # 随机选择要mask的通道（排除可能的全零通道）
        #         non_zero_channels = torch.where(z[b, 0].sum(dim=1) != 0)[0]
        #
        #         if len(non_zero_channels) > 1:  # 确保至少保留一个通道
        #             num_to_mask = max(1, int(len(non_zero_channels) * 0.1))
        #
        #             # 直接对non_zero_channels进行随机排列，然后取前num_to_mask个
        #             channels_to_mask = non_zero_channels[torch.randperm(len(non_zero_channels))[:num_to_mask]]
        #
        #             # 一次性置零所有选中的通道
        #             z_masked[b, :, channels_to_mask, :] = 0
        #     # mask版本的前向
        #     outputs_masked = self.backbone(z_masked, **kwargs)
        # else:
        #     outputs_masked = (None, None)
        
        # 返回原始输出和修改后的输出，用于计算鲁棒性损失
